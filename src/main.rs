...
async fn main() -> Result<(), GenericError> {
    ...
    for frontend in config.frontends.clone() {
        ...
        if frontend.tls {
            ...
            listener = tokio::spawn(async move {
                if let Err(e) =
                    proxy_from_https(shared_manager, tls_acceptor, frontend.clone().name, addr)
                        .await
                {
            ...
            // Frontend http
            listener = tokio::spawn(async move {
                if let Err(e) = proxy_from_http(shared_manager, frontend.clone().name, addr).await {
                    eprintln!("[Error] Frontend {} crashed: {}", frontend.name, e);
                }
            });
        }
    ...
    listener = tokio::spawn(async move {
        let shared_manager = shared_manager.clone();
        if let Err(e) = apirest_http(frontend_name.clone(), addr, shared_manager.clone()).await {
